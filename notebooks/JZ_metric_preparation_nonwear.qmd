---
title: "Preparation of metrics for participant-day analyses - without removing non-wear times"
...
This document collects the raw light exposure data for all three wearing positions through the `melidosData` package. In this sensitivity scenario, documented non-wear and diary-defined sleep are retained. The data are preprocessed to
...
#| filename: Overview before out-of-range removal
light |> 
  ...
  tab_header(
    "Summary of participant-days",
    subtitle = "Documented non-wear and diary-defined sleep retained"
  )
```

## Preprocessing step 1

In this step, documented non-wear and diary-defined sleep remain in the data. Only measurements ≥ 1.0*10^5 lx melanopic EDI are removed, together with concurrent measurements from the other positions.

```{r}
#| label: preprocessing-step1
#| filename: Remove out-of-range measurements
light <- 
light |> 
  imap(\(x, idx) x |> 
...
#| filename: Overview after out-of-range removal
light_flat |> 
  ...
  tab_header(
    "Summary of remaining participant-days after out-of-range removal",
    subtitle = "Documented non-wear and diary-defined sleep retained"
  )
```
...
save(metrics, file = here("data/prepared_metrics_nonwear.RData"))
```
